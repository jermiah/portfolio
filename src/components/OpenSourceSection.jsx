...
const OpenSourceSection = () => {
  ...
  const opensource = t('opensource', { returnObjects: true, defaultValue: null });

  // Don't render if opensource section doesn't exist
  if (!opensource || !opensource.items) {
    return null;
  }

  return (
    <Section id="opensource" title={opensource.title}>
      <div className="grid md:grid-cols-2 gap-6">
        {opensource.items.map((item, index) => (
          <motion.div
            key={index}
      ...
      {opensource.ongoingWork && opensource.ongoingWork.length > 0 && (
        <div className="mt-12">
          <h3 className="text-2xl font-bold text-gray-900 dark:text-gray-100 mb-6">
            Ongoing Work
          </h3>
          <div className="grid md:grid-cols-2 gap-6">
            {opensource.ongoingWork.map((item, index) => (
              <motion.div
                key={index}
                initial={{ opacity: 0, y: 30 }}
                whileInView={{ opacity: 1, y: 0 }}
